...
)
{
	switch( uMsg )
	{
		case WM_INITDIALOG:
		{
			...
			SetDlgItemTextW( hDlg, IDC_ASSERT_MSG_CTRL, nowide::widen(g_Info.m_pExpression).c_str() );
			SetDlgItemTextW( hDlg, IDC_FILENAME_CONTROL, nowide::widen(g_Info.m_pFilename).c_str() );
...
DBG_INTERFACE bool DoNewAssertDialog( const char *pFilename, int line, const char *pExpression )
{
	...
	{
		int result = MessageBoxW( NULL, nowide::widen(pExpression).c_str(), L"Assertion Failed", MB_SYSTEMMODAL | MB_CANCELTRYCONTINUE );
